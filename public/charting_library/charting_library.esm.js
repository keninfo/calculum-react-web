...
const De = class {
  ...
  async _create() {
    ...
    if (null === c) throw new Error(`There is no such element - #${this._options.container}`)
    ;(c.innerHTML = a), (this._iFrame = c.querySelector(`#${this._id}`))
    const d = this._iFrame
    r && (await this._innerWindowEvent('sameOriginLoad')),
      n &&
        (d.contentWindow
          ? (d.contentWindow.document.open(), d.contentWindow.document.write(s), d.contentWindow.document.close())
          : console.warn(
      ...
      (this._options.autosize || this._options.fullscreen) &&
        ((d.style.width = '100%'), this._options.fullscreen || (d.style.height = '100%')),
