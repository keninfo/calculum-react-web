  ...
  return (
    <li>
      <button onClick={() => handleSelection(days)}>
        <p
          className={`${selected === selection ? 'border-b-2 border-carmesi text-carmesi' : 'text-white'} h-5 cursor-pointer px-1 pb-2`}
        >
          {string}
        </p>
      </button>
  ...
  return (
    <ul className="flex items-center justify-center space-x-3 p-5 text-xs">
      <li onClick={() => setStudyCase(1)} className="cursor-pointer">
        <BearAttack className={`${selected === 0 ? 'fill-carmesi' : 'fill-white'} h-5`} />
      </li>
      <li onClick={() => setStudyCase(2)} className="cursor-pointer">
        <BearPassive className={`${selected === 1 ? '!fill-carmesi' : '!fill-white'} mx-2 h-5`} />
      </li>
